...
class ProfileController(
    ...
    fun baseEditProfile(@RequestBody editProfileDto: BaseProfileEditDto): ResponseEntity<*> {
        return try{
            val user: Person = personService.getCurrentPerson()
            if(editProfileDto.login != null){
                user.login = editProfileDto.login!!
            }
            if(editProfileDto.name != null){
                user.name = editProfileDto.name!!
            }
            if(editProfileDto.dateOfBirth != null){
                user.birthDate = editProfileDto.dateOfBirth!!
            }
            if(editProfileDto.status != null){
                user.status = editProfileDto.status!!
            }
            if(editProfileDto.bio != null){
